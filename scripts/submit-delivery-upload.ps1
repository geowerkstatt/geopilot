...
<#
.SYNOPSIS
    Delivers files to geopilot as a machine client, on an installation that keeps its uploads in an object storage.
...
.DESCRIPTION
    Runs the machine delivery end to end, the way a client of such an installation has to:
      ...
      2. POST {ApiUrl}/api/v2/upload        announces the files, answers one upload URL per file
      3. PUT  {uploadUrl}                   uploads each file to its URL (a presigned blob URL)
      4. POST {ApiUrl}/api/v1/submission    starts the attempt for the mandate key and the upload
      5. GET  {Location of step 4}          polls the attempt until it is delivered, rejected or failed

    An installation that writes its uploads through the API refuses step 4 with 400 and asks for the
    multipart form instead; use submit-delivery-files.ps1 there. Which of the two applies is a property
    of the installation, not of the mandate, so a client is set up once with the matching script.
...
.EXAMPLE
    ...
    ./scripts/submit-delivery-upload.ps1 -MandateKey av-2026 -File .\lieferung.xtf -SkipCertificateCheck

.EXAMPLE
    ./scripts/submit-delivery-upload.ps1 -ApiUrl https://geopilot.example.ch -TokenUrl https://idp.example.ch/oauth2/token -ClientId my-client -ClientSecret '...' -MandateKey av-2026 -File .\a.xtf, .\b.xtf -Comment 'Nachlieferung'
#>
[CmdletBinding()]
param(
...
$duplicate = $files | Group-Object Name | Where-Object Count -gt 1 | Select-Object -First 1
if ($duplicate) {
    throw "Two files are named <$($duplicate.Name)>; an upload is declared by file name."
}
...
function Invoke-UploadAnnouncement([string]$Body) {
    # This is the call an installation turns down when it is busy, not the submission below: the files are
    # counted here. 429 comes from the upload rate limit, which is one window for the whole installation, so
    # a client that delivers on a schedule meets it sooner or later and simply waits.
    for ($attempt = 1; ; $attempt++) {
        try {
            return Invoke-RestMethod -Method Post -Uri "$ApiUrl/api/v2/upload" -Headers (Get-AuthorizationHeader) -ContentType 'application/json' -Body $Body
        }
        catch {
            $httpResponse = $_.Exception.Response
            if ($null -eq $httpResponse) { throw }

            if ($httpResponse.StatusCode -eq [System.Net.HttpStatusCode]::TooManyRequests -and $attempt -lt 5) {
                ...
                Write-Host "The installation takes no further upload right now, retrying in $seconds s (attempt $attempt of 5)."
                Start-Sleep -Seconds $seconds
                continue
            }

            # An installation at its capacity answers 400 here, which is not repeated: it names what is in the
            # way, and a client that retries it blindly would hammer a full installation.
            throw "Announcing the upload was refused with HTTP $([int]$httpResponse.StatusCode): $($_.ErrorDetails.Message)"
        }
    }
}

function Invoke-Submission([string]$Body) {
    # The files are already uploaded at this point, so this route does not turn the attempt down for capacity.
    # A 503 here comes from a proxy or an instance going down, which is worth repeating unchanged.
    for ($attempt = 1; ; $attempt++) {
        try {
            $headers = $null
            $response = Invoke-RestMethod -Method Post -Uri "$ApiUrl/api/v1/submission" -Headers (Get-AuthorizationHeader) -ContentType 'application/json' -Body $Body -ResponseHeadersVariable headers
...
# 2. Announce the files: name and size, answered with one upload URL per file.
Write-Host "Announcing $($files.Count) file(s) to $ApiUrl ..."
$announcement = @{ files = @($files | ForEach-Object { @{ fileName = $_.Name; size = $_.Length } }) } | ConvertTo-Json -Depth 5
$upload = Invoke-UploadAnnouncement $announcement

# 3. Upload each file to its URL. The URL itself authorizes the PUT, so no bearer header goes here;
#    the blob type header is what an Azure blob storage requires for a single PUT.
foreach ($entry in $upload.files) {
    $local = $files | Where-Object Name -eq $entry.fileName | Select-Object -First 1
    Write-Host "Uploading $($local.Name) ($($local.Length) bytes) ..."
    Invoke-RestMethod -Method Put -Uri $entry.uploadUrl -InFile $local.FullName -ContentType 'application/octet-stream' -Headers @{ 'x-ms-blob-type' = 'BlockBlob' } | Out-Null
}

# 4. Start the attempt. Only the fields that were given go into the request; the mandate decides which
#    of the optional ones are forbidden, allowed or required.
$request = @{ mandateKey = $MandateKey; uploadId = $upload.uploadId }
if ($Comment) { $request.comment = $Comment }
if ($null -ne $PartialDelivery) { $request.partialDelivery = $PartialDelivery }
if ($null -ne $PrecursorDeliveryId) { $request.precursorDeliveryId = $PrecursorDeliveryId }

$submission = Invoke-Submission ($request | ConvertTo-Json)
Write-Host "Accepted as attempt $($submission.Response.id); status at $($submission.StatusUrl)"

# 5. Poll until the attempt has ended.
