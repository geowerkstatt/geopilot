...
- The Swagger UI uses the configured `Auth:AuthorizationUrl` and `Auth:TokenUrl` even when `Auth:SwaggerAdditionalScopes` is empty. Deployments whose identity provider puts the audience into the token on its own need no extra scope, and those endpoints were silently ignored in favour of the discovery document. Deployments that set all three are unaffected.
