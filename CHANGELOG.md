...
- Pipeline step `input` is now a map from process parameter name to value, replacing the previous list of `from`, `take` and `as` entries. Each value is either a literal or a `${step_output(stepId.outputName)}` reference, and existing pipeline definitions must be updated to the new form.
