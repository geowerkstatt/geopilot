...
- The XTF validation can check a delivery against the models it brings along: the new `modelFiles` input of the validation step passes the delivered models to the ilitools-wrapper, which stores them next to the transfer file under names of its own, so a repository index (`ilidata.xml`, `ilisite.xml`, `ilimodels.xml`) cannot be smuggled in through an upload. The pipeline decides what counts as a model by what it wires into the input; the built-in pipelines select the `.ili` files of the upload with a file matcher step. The delivered models only take part when `modelDirs` reaches them through the `%ITF_DIR` placeholder (or is not configured at all); `%ITF_DIR` belongs at the end of `modelDirs` so the configured repositories win and a delivery only fills real gaps. The mandate's file types must allow `.ili` for such uploads. INTERLIS 1 deliveries are supported: the `.itf` extension of the transfer file now reaches the tool, which derives its INTERLIS 1 semantics from it. The clients `IIlivalidatorClient` and `IIli2GpkgClient` accept optional `modelFiles` on their operations. Requires the first ilitools-wrapper release containing geowerkstatt/ilitools-wrapper#17.
