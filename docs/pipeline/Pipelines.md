...
- `Download`: Die Daten werden zum Download bereitgestellt, und können von den Anwendern heruntergeladen werden. Der Wert muss ein `IPipelineFile` oder eine Sammlung davon (`IEnumerable<IPipelineFile>`) sein. Der deklarierte Typ der Property wird beim Laden geprüft: Eine Property, die gar keine Datei liefern kann, verhindert den Start. Ist der Wert zur Laufzeit nicht gesetzt (`null`), schlägt der Schritt fehl.
- `Delivery`: Die Daten werden in der Lieferung mit abgegeben. Der Wert muss ein `IPipelineFile` oder eine Sammlung davon (`IEnumerable<IPipelineFile>`) sein; auch hier wird der deklarierte Typ beim Laden geprüft. Ist der Wert zur Laufzeit nicht gesetzt (`null`), wird nichts geliefert, und der Schritt schlägt anders als bei `Download` nicht fehl. Die Lieferung enthält nur **genau** die Dateien, die von einem Schritt als `Delivery` getaggt wurden. Um hochgeladene Dateien in der Lieferung zu berücksichtigen, wird üblicherweise ein Matcher-Schritt verwendet, der die Uploads als `${upload()}` erhält und seine Treffer und optional seine `UnmatchedFiles` als `Delivery` taggt. Der Asset-Typ einer gelieferten Datei ergibt sich aus ihrer Herkunft: kam sie als Upload in die Pipeline, wird sie als `PrimaryData` geführt, wurde sie innerhalb der Pipeline erzeugt oder bearbeitet, als `ProcessedData`. Enthält eine Lieferung keine einzige `Delivery`-getaggte Datei, wird die Deklaration mit einem Fehler abgelehnt.
...
- `Visualization`: Markiert die Ausgabe als Visualisierung. Der zugewiesene Wert muss ein `IVisualization`-Envelope sein (keine Datei); wie bei `Download` wird der deklarierte Typ beim Laden geprüft, und ein zur Laufzeit nicht gesetzter Wert (`null`) lässt den Schritt fehlschlagen. Die Runtime serialisiert ihn zu JSON (camelCase, `null`-Felder werden weggelassen), legt ihn im dedizierten Visualisierungs-Store ab und vermerkt eine Referenz auf dem Schritt, ausgeliefert wird er über den Visualisierungs-Endpunkt. Das JSON ist ein selbstbeschreibender Envelope `{ "type": <Diskriminator>, "data": <Payload> }`: `type` wählt die Frontend-Komponente, `data` ist der von ihr gerenderte Payload. Das konkrete `data`-Format ist prozessor-spezifisch und bei der jeweiligen Prozessor-Doku beschrieben. Diese Ausgabeaktion ist **eingebauten Prozessoren vorbehalten**: `IVisualization` ist nicht Teil des Plugin-Vertrags, und die darstellende Komponente liegt im geopilot-Frontend (siehe [Plugin-System](pluginSystem.md)).
...
- `pipelines`: Liste von Pipelines, welche die Verarbeitungsschritte beschreiben.
  ...
  - `pipelines[0].steps`: Liste der Schritte, die in der Pipeline ausgeführt werden. In diesem Beispiel ist der erste Schritt (`steps[0]`) der XTF Matcher, welcher die hochgeladenen Dateien filtert, und der zweite Schritt (`steps[1]`) die XTF Validierung.
    ...
    - `pipelines[0].steps[X].input`: Definiert, wie der Schritt an seine Eingabedaten kommt. Der Input ist eine Zuordnung (Map) von Prozessparameter-Namen zu Werten: Der Schlüssel ist der Name des Run-Methoden-Parameters, dem der Wert übergeben wird, der Wert ist die Quelle für diesen Parameter. Es können mehrere Parameter definiert werden, welche alle in der Dokumentation des verwendeten Prozessors beschrieben sein müssen. In diesem Beispiel wird im Validierungsschritt (`steps[1]`) der Parameter `transferFile` gesetzt, welcher den Output `XtfFiles` des Schrittes `xtf_matching` bezieht.
      ...
      - Mehrere Quellen auf denselben Parameter: Als Wert kann eine YAML-Liste von Einträgen (Referenzen und/oder Literale) angegeben werden, welche zu einem einzigen Parameter zusammengeführt werden. Bei einem Listen-Parameter (Array bzw. `params`) trägt jede Quelle ihre Werte bei. Bei einem **einwertigen** Parameter werden die Quellen genauso zusammengeführt, es muss danach aber genau ein Wert übrig bleiben: Quellen, die nichts liefern (eine leere Liste, ein `null` oder ein übersprungener Schritt), zählen nicht mit, und bleiben zwei oder mehr Werte übrig, schlägt der Schritt fehl. Das gilt auch für mehr als zwei Quellen; die Definition muss dann aber selbst dafür sorgen, dass höchstens eine davon einen Wert liefert, denn geprüft wird das erst zur Laufzeit. Damit lässt sich "die Datei kommt aus der einen **oder** der anderen Quelle" ausdrücken, ohne den Parameter auf eine Liste umzustellen.
      - Referenz auf einen übersprungenen Schritt: Ein übersprungener Schritt (siehe [PRE-Condition](#pre-condition)) führt keinen Prozess aus und liefert deshalb keinen Wert. Eine Referenz darauf ist trotzdem erlaubt und steuert einfach nichts bei: In einer Liste von Quellen entfällt der Eintrag, bei einem Listen-Parameter als einzige Quelle bleibt der Parameter leer, und bei einem einzelnen Parameter wird `null` übergeben, sofern der Parameter das zulässt. Damit lässt sich ein optionaler Zwischenschritt abbilden, ohne die nachfolgenden Schritte zu verdoppeln: Ein Entpack-Schritt wird übersprungen, wenn kein Archiv geliefert wurde, und der nachfolgende Schritt bezieht seine Dateien aus dem Upload **und** aus dem entpackten Archiv. Lässt ein einzelner Parameter kein `null` zu, schlägt der Schritt weiterhin fehl, mit einer Meldung, die den übersprungenen Schritt benennt.
...
- **Skipped**: Schritt wird aufgrund einer `skip_conditions`-Condition übersprungen und nicht ausgeführt. In der Pipeline wird ein übersprungener Schritt und ein erfolgreich abgeschlossener Schritt gleich behandelt. Es ist somit möglich, dass ein Schritt übersprungen wird, und die Pipeline trotzdem mit Erfolg abschliesst. Ein übersprungener Schritt liefert keine Outputs: Nachfolgende Schritte dürfen seine Outputs referenzieren, erhalten dann aber keinen Wert (siehe [Aufbau der Pipeline-Definition](#wie-ist-eine-pipeline-definition-aufgebaut)), und in einer Condition wird ein solcher Output als `null` ausgewertet.
