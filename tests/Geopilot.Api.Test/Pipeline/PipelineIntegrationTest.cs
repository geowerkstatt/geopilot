...
public class PipelineIntegrationTest
{
    ...
    private static string appConfig = "{\"Pipeline\":{\"Definition\":\"myPipeline.yaml\",\"Plugins\":[\"TestData/Pipeline/Plugins/Geopilot.ProcessorPluginA.dll\"]},\"Validation\":{\"InterlisCheckServiceUrl\":\"http://localhost/\"}}";
    ...
    [TestMethod]
    public async Task RunPluginPipeline()
    {
        var uploadStepId = "upload";
        var validationStepId = "validation";
        var pluginStepAId = "plugin_step_a";
        var pluginStepBId = "plugin_step_b";
        var uploadedFileAttribute = "ili_file";

        PipelineFactory factory = CreatePipelineFactory("pluginPipeline");
        using var pipeline = factory.CreatePipeline("plugin_pipeline");
        ...
        Assert.IsTrue(stepResults.ContainsKey(pluginStepAId));
        var pluginAStepResult = stepResults[pluginStepAId];
        Assert.HasCount(2, pluginAStepResult.Outputs, "plugin step A has not the expected number of data");

        Assert.IsTrue(stepResults.ContainsKey(pluginStepBId));
        var pluginBStepResult = stepResults[pluginStepBId];
        Assert.HasCount(1, pluginBStepResult.Outputs, "plugin step B has not the expected number of data");
    }
