...
public class PreflightBackgroundServiceTest
{
    ...
    public async Task ProcessRequestAsyncHappyPath()
    {
        ...
        pipelineFactoryMock.Setup(x => x.CreatePipeline(pipelineId, It.IsAny<IPipelineTransferFile>())).Returns(pipeline.Object);
    ...
    public async Task ProcessRequestAsyncSetsFailedOnPreflightFailure()
    {
        ...
        jobStoreMock.Setup(x => x.SetJobStatus(jobId, Status.Failed)).Returns(cloudJob with { Status = Status.Failed });

        await service.ProcessRequestAsync(new PreflightRequest(jobId, 1, null));

        jobStoreMock.Verify(x => x.SetJobStatus(jobId, Status.Failed), Times.Once);
        cloudStorageServiceMock.Verify(x => x.DeletePrefixAsync($"uploads/{jobId}/"), Times.Once);
    }

    [TestMethod]
    public async Task ProcessRequestAsyncSetsFailedOnThreatDetected()
    {
        var jobId = Guid.NewGuid();
        var cloudJob = new ValidationJob(jobId, null, null, null, ImmutableDictionary<string, ValidatorResult?>.Empty, Status.VerifyingUpload, DateTime.Now, UploadMethod.Cloud);
        ...
        jobStoreMock.Setup(x => x.GetJob(jobId)).Returns(cloudJob);
        cloudOrchestrationServiceMock.Setup(x => x.RunPreflightChecksAsync(jobId))
            .ThrowsAsync(new CloudUploadPreflightException(PreflightFailureReason.ThreatDetected, "Threat found."));
        cloudStorageServiceMock.Setup(x => x.DeletePrefixAsync($"uploads/{jobId}/")).Returns(Task.CompletedTask);
        jobStoreMock.Setup(x => x.SetJobStatus(jobId, Status.Failed)).Returns(cloudJob with { Status = Status.Failed });
