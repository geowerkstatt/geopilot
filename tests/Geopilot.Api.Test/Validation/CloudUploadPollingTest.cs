...
public class CloudUploadPollingTest
{
    ...
    private void SetupSuccessfulPreflight(Guid jobId, Mandate mandate, User user)
    {
        ...
        pipelineFactoryMock.Setup(x => x.CreatePipeline(mandate.PipelineId!, It.Is<IPipelineTransferFile>(f => f.OriginalFileNameWithoutExtension == "test" && f.FilePath == "path/to/random.xtf"))).Returns(pipeline.Object);
    }
}
