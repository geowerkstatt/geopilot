...
public class ZipPackageProcessTest
{
    ...
    public async Task SunnyDay()
    {
        ...
        var processResult = await process.RunAsync(new IPipelineFile[] { uploadFile }, CancellationToken.None);
    ...
    [TestMethod]
    public async Task NoArchiveFileNameProvided()
    {
        var pipelineFileManager = new PipelineFileManager(Path.GetTempPath(), "ZipPackageProcess");
    ...
    public async Task NoInputFilesProvided()
    {
        var pipelineFileManager = new PipelineFileManager(Path.GetTempPath(), "ZipPackageProcess");
        var process = new ZipPackageProcess(null, pipelineFileManager, Mock.Of<ILogger<ZipPackageProcessTest>>());
        var exception = await Assert.ThrowsAsync<ArgumentException>(() => process.RunAsync(Array.Empty<IPipelineFile>(), CancellationToken.None));
    ...
    public async Task AllInputFilesAreNull()
    {
        var pipelineFileManager = new PipelineFileManager(Path.GetTempPath(), "ZipPackageProcess");
        var process = new ZipPackageProcess(null, pipelineFileManager, Mock.Of<ILogger<ZipPackageProcessTest>>());
        var processResult = await process.RunAsync(new IPipelineFile?[] { null, null, null }, CancellationToken.None);
        Assert.IsNotNull(processResult);
        Assert.IsNotNull(processResult.StatusMessage);
    ...
    public async Task MixedNullAndValidInputFiles()
    {
        ...
        var uploadFile = new PipelineFile("TestData/UploadFiles/RoadsExdm2ien.xtf", "RoadsExdm2ien.xtf");
        var processResult = await process.RunAsync(new IPipelineFile?[] { null, uploadFile, null }, CancellationToken.None);
        Assert.IsNotNull(processResult);
        Assert.IsNotNull(processResult.ZipPackage);
        Assert.IsNotNull(processResult.StatusMessage);
        var zipArchive = processResult.ZipPackage;
    ...
    public async Task PreservesOriginalRelativePath()
    {
        ...
        var processResult = await process.RunAsync(new IPipelineFile[] { rootFile, nestedFile }, CancellationToken.None);
        var zipArchive = processResult.ZipPackage;
        Assert.IsNotNull(zipArchive);

        using var zipStream = await zipArchive.OpenReadAsync();
    ...
    public async Task SameFileNameInDifferentDirectoriesNoDuplicateWarning()
    {
        ...
        var processResult = await process.RunAsync(new IPipelineFile[] { file1, file2 }, CancellationToken.None);
        var zipArchive = processResult.ZipPackage;
        Assert.IsNotNull(zipArchive);
