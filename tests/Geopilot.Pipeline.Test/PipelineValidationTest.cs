...
public class PipelineValidationTest
{
    ...
    [DataRow("invalidStepInputFromReference_01", new string[] { "PipelineConfig: Step 'validation' input 'iliFile' references 'zip.zip_package', which is not an output of an earlier step." }, DisplayName = "Step input references a later step")]
    [DataRow("invalidStepInputFromReference_02", new string[] { "PipelineConfig: Step 'validation' input 'iliFile' references 'invalidUploadStep.ili_file', which is not an output of an earlier step." }, DisplayName = "Step input references an unknown step")]
    [DataRow("invalidStepInputTakeReference", new string[] { "PipelineConfig: Step 'zip_package' input 'input' references 'validation.invalid_reference', which is not an output of an earlier step." }, DisplayName = "Step input references an unknown output")]
    ...
    [DataRow("invalidStepInputUnsupportedReference", new string[] { "PipelineConfig: Step 'validation': Reference '${uploads}' is not supported. Use ${step_output(stepId.outputName)} or ${file(path)}." }, DisplayName = "Step input has an unsupported reference")]
    [DataRow("invalidStepInputNestedSequence", new string[] { "PipelineConfig: Step 'validation': Input 'iliFile': a list must not contain another list. Nested lists are not supported." }, DisplayName = "Step input has a nested list")]
    [DataRow("invalidStepInputSequenceReference", new string[] { "PipelineConfig: Step 'zip_package' input 'input' references 'validation.invalid_reference', which is not an output of an earlier step." }, DisplayName = "Step input sequence references an unknown output")]
