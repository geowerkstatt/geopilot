...
public interface IIli2GpkgClient
{
    ...
    /// them next to the transfer files under names of its own, so a repository index cannot be smuggled in, which
    /// makes this the channel for models from an unreviewed source such as an upload. They become visible through
    /// the placeholder <c>%XTF_DIR</c> in <see cref="Ili2GpkgArgs.ModelDirs"/> (or through the tool default when no
    /// model dirs are set); its position decides the precedence against the configured repositories.
    /// </param>
    /// <param name="cancellationToken">Token to cancel the operation.</param>
    /// <returns>An <see cref="Ili2GpkgResult"/> indicating success and the ili2gpkg log content.</returns>
    ...
    /// <param name="modelFiles">
    /// Optional single INTERLIS model files (<c>.ili</c>) delivered with the request, stored in the working directory
    /// of the operation under names of its own and visible through the placeholder <c>%XTF_DIR</c> in
    /// <see cref="Ili2GpkgArgs.ModelDirs"/> (or through the tool default when no model dirs are set).
    /// </param>
    /// <param name="cancellationToken">Token to cancel the operation.</param>
    /// <returns>An <see cref="Ili2GpkgResult"/> indicating success and the ili2gpkg log content.</returns>
