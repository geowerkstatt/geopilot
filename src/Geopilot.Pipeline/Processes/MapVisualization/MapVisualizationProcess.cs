...
internal class MapVisualizationProcess
{
    ...
    public async Task<Dictionary<string, object?>> RunAsync(IPipelineFile errorLogXtf)
    {
        ...
        // The client draws layers in array order (later = on top), so the colored map is listed last to stay the
        // default base map; the grey map sits beneath it and can be selected in the layer switcher.
