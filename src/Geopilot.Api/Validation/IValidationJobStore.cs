...
namespace Geopilot.Api.Validation
{
    ...
    public interface IValidationJobStore
    {
        ...
        /// <see cref="IPipeline"/> instances ready for processing.
        /// It is expected that consumers deliver an <see cref="ValidatorResult"/> for each consumed <see cref="IPipeline"/>
        /// by calling <see cref="AddValidatorResult(IPipeline, ValidatorResult)"/>.
        /// </summary>
        ChannelReader<IPipeline> ValidationQueue { get; }
        ...
        /// <param name="pipeline">The pipeline to run. Cannot be null.</param>
        ...
        /// <exception cref="ArgumentException">If no job with the <paramref name="jobId"/> was found.</exception>
        /// <exception cref="ArgumentException">If <paramref name="pipeline"/> is <see langword="null"/>.</exception>
        /// <exception cref="InvalidOperationException">If the status of the job is not <see cref="Status.Ready"/>.</exception>
        ValidationJob StartJob(Guid jobId, IPipeline pipeline, int mandateId);

        /// <summary>
        /// Adds the <paramref name="result"/> to the job associated with the specified <paramref name="pipeline"/>.
        /// </summary>
        ...
        /// <param name="pipeline">The pipeline that produced the result and is associated with a job.</param>
        ...
        /// <exception cref="ArgumentException">If the specified <paramref name="pipeline"/> is not associated with any job.</exception>
        /// <exception cref="InvalidOperationException">If the status of the job is not <see cref="Status.Processing"/>.</exception>
        ValidationJob AddValidatorResult(IPipeline pipeline, ValidatorResult result);
