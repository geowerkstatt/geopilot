...
public class CsvValidator : IValidator
{
    ...
    private const string SampleLogsPath = "SampleLogs/CSV";
    private static readonly ICollection<string> supportedFileExtensions = new List<string> { ".csv" };
    ...
    public async Task<ValidatorResult> ExecuteAsync(CancellationToken cancellationToken)
    {
        ...
        if (string.Equals(originalFileName, "digitised_surrogate_tech_acq_metadata_v1_TESTBATCH000_valid.csv", StringComparison.OrdinalIgnoreCase))
        {
        ...
        if (string.Equals(originalFileName, "digitised_surrogate_tech_acq_metadata_v1_TESTBATCH000_error.csv", StringComparison.OrdinalIgnoreCase))
        {
