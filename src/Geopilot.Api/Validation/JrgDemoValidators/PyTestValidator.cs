...
public class PyTestValidator : IValidator
{
    public string Name => "PyTest";

    private const string SampleLogsPath = "SampleLogs/PyTest";
    private static readonly ICollection<string> supportedFileExtensions = new List<string> { ".gpkg" };
    ...
    public async Task<ValidatorResult> ExecuteAsync(CancellationToken cancellationToken)
    {
        ...
        if (string.Equals(originalFileName, "naturereigniskataster_v1_0_2056_valid.gpkg", StringComparison.OrdinalIgnoreCase))
        {
        ...
        if (string.Equals(originalFileName, "naturereigniskataster_v1_0_2056_error.gpkg", StringComparison.OrdinalIgnoreCase))
        {
